...
- Thread agent switches are first-class: `PATCH /threads/:id` can change `agent`, clears adapter-specific live session state, resets incompatible model/effort/permission config, preserves the worktree and transcript, and inserts a visible system boundary so the next turn starts fresh under the new agent
...
- Real-time streaming via ephemeral WebSocket deltas for live text/tool updates; thread-level run metrics are persisted on `threads` so StickyRunBar usage survives refresh/reconnect; complete messages persisted to SQLite with WAL mode, seq-based replay on reconnect; cross-client sync via `thread_updated` WS broadcasts
