...
- TodoWrite rendering: latest TodoWrite renders as prominent card with all tasks, per-task status (✓ completed/▸ running/○ queued), progress bar, ARIA roles; Codex now streams canonical `{todos}` snapshots with synthesized `in_progress` status during live updates, while `parseTodos()` still accepts legacy Codex `{items}` payloads plus Claude stringified `todos/items`, bare arrays, and `title`-only items; `TodoItemList` is shared across the latest card, inline renderer, and pinned panel; prior TodoWrites collapse to expandable summary lines; `latestTodos` hydrated from REST history with streaming race guard
...
- DiffRenderer: Myers LCS diff algorithm (`lib/diffCompute.ts`) with line numbers, syntax highlighting, context lines, empty-string guards, and trailing-newline normalization; trims unchanged prefix/suffix so small edits inside large files keep precise `+/-` counts and line numbers, then falls back only for genuinely large rewrites; mobile hides line numbers <640px; fallback semantic colors when Shiki unavailable
...
- Context window indicator: token usage (input+output) vs model context window shown as a color-coded progress bar when the model reports context size, with a token-only fallback for Codex turns that only expose totals; uses **per-request** tokens from `message_start` stream events (not cumulative `modelUsage` which inflates across turns); `Math.max` for contextWindow to prevent sub-agent regression; primary-model filtering via `parent_tool_use_id === null`
...
- **QA testing from worktrees**: You CANNOT test against the already-running main-branch instance. Each worktree gets its own port (via hash), so you must `cd` into the worktree, build the client (`bun run --filter client build`), and start a fresh server from the repo root with `ORCHESTRA_ALLOW_NESTED=1`. Always set `ORCHESTRA_DATA_DIR` to a worktree-local path such as `.orchestra-worktree` (or rely on the new Orchestra-managed default, which does that for you) so nested QA does not touch `~/.orchestra/`. Only then browse to the worktree's port for QA.
