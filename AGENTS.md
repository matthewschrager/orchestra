...
- TodoWrite rendering: latest TodoWrite renders as prominent card with all tasks, per-task status (✓ completed/▸ running/○ queued), progress bar, ARIA roles; prior TodoWrites collapse to expandable summary lines; `parseTodos()` normalizes Claude and Codex payload variants, including stringified `todos/items` arrays, bare arrays, and `title`-only items; `latestTodos` hydrated from REST history with streaming race guard
