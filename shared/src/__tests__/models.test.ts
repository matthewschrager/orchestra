...
describe("getModelOptions", () => {
  ...
  test("returns codex models", () => {
    ...
    expect(options.some((o) => o.value === "o3")).toBe(true);
    expect(options.some((o) => o.value === "o4-mini")).toBe(true);
  });
...
describe("isModelSupported", () => {
  ...
  test("returns false for wrong agent", () => {
    expect(isModelSupported("claude", "o3")).toBe(false);
...
describe("getModelLabel", () => {
  test("returns display name for valid model", () => {
    expect(getModelLabel("claude", "claude-opus-4-6")).toBe("Opus");
    expect(getModelLabel("claude", "claude-sonnet-4-6")).toBe("Sonnet");
    expect(getModelLabel("codex", "o3")).toBe("o3");
  });
